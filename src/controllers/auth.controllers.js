...
const generateAccessAndRefreshTokens = async (userId)=>{
    try {
        ...
        throw new ApiError(
            500,
            "Something went wrong in the generating access and refresh token"
        )
    }
}
...
const registerUser = asyncHandler(async (req , res) =>{
    ...
    if (existedUser){
        throw new ApiError(409 , "User with the email or user already exists" , [])
    }
    ...
    return res
      ...
      .json(
        new ApiResponse(
            200,
            {},
            "Password reset mail has been sent om your mail id"
        )
      )
...
const resetForgotPassword = asyncHandler(async(req , res) => {
    ...
    if (!user){
        throw new ApiError(489 , "Invalid token or token is expired")
    }
